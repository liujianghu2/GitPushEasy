...
┌────────┬──────────────────────────────────────┬──────────────────┐
│        │  晚上好,准备上传新项目吗?            │  本次变更        │
│ 小白推送│  三步完成:选择文件夹、确认变更、推送  │  ＋ 新增    12   │
│        │                                      │  ✎ 修改     4   │
│ ▸ 首页 │  ① 选择文件夹 ─ ② 查看变更 ─ ③ 上传完成│  − 删除     1   │
│   项目 │                                      ├──────────────────┤
│  上传记录│  选择本地文件夹                     │  部分文件预览     │
│        │  [📁 D:\Projects\MyWebsite 重新选择] │  README.md    +  │
│        │                                      │  src/App.tsx  ✎  │
│ Git小课堂│  仓库设置                            ├──────────────────┤
│ 命令速查│  [仓库▾] [可见性▾] [目标分支▾]        │  上传方式        │
│   设置 │                                      │  ◉ 合并到 main   │
│        │  ✓ 已连接 GitHub                     │  ○ 新建分支上传   │
│ ~山峦~ │  ✓ 未发现敏感文件                    │                  │
│        │  ✓ .gitignore 已就绪                 │                  │
...
> **关于"账号 + 密码"登录:本工具刻意不提供。**
> GitHub 自 2021-08-13 起已彻底停用密码推送 —— 命令行、GitHub Desktop、VS Code
> 用密码都会收到同样的拒绝。留一个注定失败的入口只会让人以为"工具坏了",
> 所以界面上不存在这个选项,而是把那三条真正可用的路做好。
